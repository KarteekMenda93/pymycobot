...
class Object_detect():
    ...
    def move(self, x, y, label):
        ...
        first_letter = self.class_name[0].lower()

        if first_letter == "s":#sportsball
            color = 0#put it in a bin D
            print(f"Moving sportsball {label} to bin D")
        elif first_letter == "v":#vase
            color = 1#put it in a sepearet bin C
            print(f"Moving vase {label} to bin C")
        else:
            color =2#put it in a seperate bin A
            print(f"Moving onion {label} to bin A")
